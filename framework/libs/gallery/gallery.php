...
class ffGalleryCollection implements Iterator {
	...
	public function loadGalleryFromMeta( $metaName ) {
		$customGallery = metaBoxManager::getMeta($metaName);
		if( !empty( $customGallery ) ) {
			$customGalleryExploded = explode(',', $customGallery);
	...
	private function _loadGallery() {
		...
		$customGallery = metaBoxManager::getMeta('gallery_images');
		if( !empty( $customGallery ) ) {
			$this->loadGalleryFromMeta( 'gallery_images');
		} else {
			$attachmentArgs = array(
					'post_type' => 'attachment',
					'numberposts' => -1,          // one attachement image per post
