...
class fOpt
{
    ...
    private static function Install() {
        ...
        require_once get_template_directory().'/install/options/install.php';
        ffOptionsInstall::defaults();
	...
	private static function Load() {
	    ...
	    $sql = "SELECT * FROM ".self::$table_name. " WHERE namespace != 'pagebuilder_templates'";
	...
	private static function LoadPagebuilderTemplates() {
	    global $wpdb; self::$table_name = $wpdb->prefix."gen_options";
	    $sql = "SELECT * FROM ".self::$table_name. " WHERE namespace = 'pagebuilder_templates'";
	    $result = mysql_query( $sql );
	    if($result == false) {
	...
	public static function GetClean( $namespace, $name ) {
		self::checkPb( $namespace, $name );
		if( isset(self::$opt[ $namespace ][ $name ]) )
			return (self::$opt[ $namespace ][ $name ]);
		else return null;
	}
	
	public static function checkPb( $namespace, $name ) {
		if( $namespace != 'pagebuilder_templates' ) return;
		
		global $wpdb; self::$table_name = $wpdb->prefix."gen_options";
		$sql = "SELECT * FROM ".self::$table_name. " WHERE namespace = 'pagebuilder_templates' AND name='".$name."' ";
		$result = mysql_query( $sql );
		if($result == false) {
			return;
		}
	...
	public static function GetNamespace( $namespace ) {
		if('pagebuilder_templates' == $namespace){
        self::LoadPagebuilderTemplates();
    }
