...
class fSidebarCollection {
	...
	private function loadSidebars() {
		...
		$sc = fOpt::get('fsidebars', 'sidebar_counter');
		if( $sc == null ) {
			$sc = 0;
			fOpt::Set('fsidebars', 'sidebar_counter', $sc);
		}
		$this->sidebar_counter = $sc;
		
		$sidebars = fOpt::GetNamespace('fsidebars');
	...
	public function  addSidebar( $name ) {
		...
		fOpt::Set('fsidebars', 'sidebar_counter', $this->sidebar_counter);
		
		$this->sidebars[ $this->sidebar_counter ] = $name;
		fOpt::Set('fsidebars', $this->sidebar_counter, $name);
	}
	
	public function editSidebar( $id, $new_name ) {
		$this->sidebars[ $id ] = $new_name;
		fOpt::Set('fsidebars', $id, $new_name );
	}
...
class fSidebarManager {
	...
	private function _getSidebarThemeName() {
		...
		else if( (is_category() && !is_single() && !is_singular() && !is_page()) || is_search() || is_tag() || is_author() || is_archive() ) {
