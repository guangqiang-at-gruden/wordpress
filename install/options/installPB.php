...
class ffPBOptionsInstall{
    ...
    function replace($content){
        ...
        $to_replace[]   = 'XXX_REQUEST_A_QUOTE_CONTACT_FORM_XXX';
        $replace_with[] = ffGetByTitle::post('WG - request quote');

        $to_replace[]   = 'XXX__PORTFOLIO_PAGINATION_3__XXX';
        $replace_with[] = ffGetByTitle::term('Portfolio Pagination 3');

        $to_replace[]   = 'XXX__BLOG_CATEGORY__XXX';
        $replace_with[] = ffGetByTitle::term('Blog Medium');
    ...
    function runSQLfromFile( $file_path ){
        ...
        $content = ffPBOptionsInstall::replace($content);
        mysql_query( $content );
    }
    
    function defaults(){
        if( fOpt::Get('fullthemeinstall', 'pagebuilder' ) ){
            return;
        }

        ffPBOptionsInstall::runSQLfromFile( dirname(__FILE__).'/data/dp_pb_exp.frs' );
