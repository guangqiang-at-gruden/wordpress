...
class pluginsInstallClass {
    ...
    static function init(){
        ...
        switch ( fOpt::Get('basicinstall', 'plugins' ) ) {
    ...
    static function redirectToInstallPlugins(){
        fOpt::Set('basicinstall', 'plugins', 'install-required-plugins-page' );
        wp_enqueue_script('jquery');
        wp_enqueue_script(
              'ff_autoinstall1',
              get_template_directory_uri().'/install/plugins/js-plugins-install-1.js',
              array('jquery'),
              md5( filemtime( get_template_directory().'/install/plugins/js-plugins-install-1.js' ) ),
              false  // into footer
        );
    }
}
